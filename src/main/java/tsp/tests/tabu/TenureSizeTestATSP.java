...
public class TenureSizeTestATSP {

	public void test() throws IOException {

		ATSPMatrixImporter ATSPMatrixImporter = new ATSPMatrixImporter();
		FileWriter fileWriter = new FileWriter("wyniki_tenure_atsp_normal.txt");
	    ...
	    for (int i = 20; i <= 120; i += 10) {
	    	...
	    	for(int k = 0; k < 15; k++) {
				...
				Tabu2OptWithAspiration algorithm = new Tabu2OptWithAspiration(graph, 10000, 1000, false, 7, 0.1, 50, 3);
				graph.setOptimalPath(algorithm.findSolution());
				
				algorithm = new Tabu2OptWithAspiration(graph, 10000, 1000, false, 3, 0.1, 50, 3);
				PRDAvgConstants[0] += graph.PRD(algorithm.findSolution());
				
				for(int j = 1; j < 4; j++) {
					algorithm = new Tabu2OptWithAspiration(graph, 10000, 1000, false, (7 + 4*j), 0.1, 50, 3);
					PRDAvgConstants[j] += graph.PRD(algorithm.findSolution());
				}
				
				for(int j = 0; j < 5; j++) {
					algorithm = new Tabu2OptWithAspiration(graph, 10000, 1000, false, Integer.valueOf((int) Math.ceil(0.4*(j+1)*Math.log(i))), 0.1, 50, 3);
					PRDAvgLogs[j] += graph.PRD(algorithm.findSolution());
					algorithm = new Tabu2OptWithAspiration(graph, 10000, 1000, false, Integer.valueOf((int) Math.ceil(0.4*(j+1)*Math.sqrt(i))), 0.1, 50, 3);
					PRDAvgSqrts[j] += graph.PRD(algorithm.findSolution());
					algorithm = new Tabu2OptWithAspiration(graph, 10000, 1000, false, Integer.valueOf((int) (0.4*(j+1)*(i))), 0.1, 50, 3);
					PRDAvgLinears[j] += graph.PRD(algorithm.findSolution());
					algorithm = new Tabu2OptWithAspiration(graph, 10000, 1000, false, Integer.valueOf((int) (0.4*(j+1)*i*i)), 0.1, 50, 3);
